...
class GateOverARepository(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        subprocess.run(["git", "init", "--quiet"], cwd=self.repository, check=True, capture_output=True)

    def run_gate(self, working_directory: Path) -> subprocess.CompletedProcess[str]:
        # git's own configuration is pinned to the null device: the gate asks
        # git which untracked files are ignored, so a machine-wide ignore
        # matching a probe file would otherwise turn a red path green here.
    ...
    def test_it_names_a_stray_root_file_and_fails_from_any_directory(self) -> None:
...
class RootLayoutProblems(unittest.TestCase):
    ...
    def test_a_stray_root_file_is_named_with_the_sentence_where_it_belongs(self) -> None:
        strays_and_homes = [
            ...
            ("claude-resume-crashed", check_root_layout.DEFAULT_HOME),
        ]
